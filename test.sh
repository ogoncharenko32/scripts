...
#   rpd_pn=$(grep -i "time exor --get-base-mac-addr'" -A3 "$fssm_path/GENERAL/general_info.txt" | awk 'NR == 3' | cut -d "'" -f 4)
#   rpd_sn=$(grep -i "time exor --get-base-mac-addr'" -A3 "$fssm_path/GENERAL/general_info.txt" | awk 'NR == 3' | cut -d "'" -f 8)
  ...
  for folder in */; do
    ...
    for inner_folder in "$folder"*; do
      ...
      if [ -d "$inner_folder" ]; then
        ...
        # journalctl --directory="$inner_folder" --output=short-full > "$journals_backup_path$inner_folder.log"
      fi
    done
  cd "$path" || continue
  echo "$PWD"
...
if [ -d "$path/../../run/log/journal" ]; then
  ...
  for folder in */; do
    ...
    journalctl --directory="$folder" --output=short-full > "$journals_backup_path$folder.log" 
  done
